...
public static class RateLimitPolicies
{
    ...
    public static IEnumerable<RateLimitRule> JobQueuePolicies() =>
    [
        // POST /jobs — producer rate limit
        new RateLimitRule
        {
            PolicyName = "job-submit",
            Limit      = 1000,
            Window     = TimeSpan.FromMinutes(1),
            Paths      = ["/api/v1/jobs"],
            Methods    = ["POST"]
        },
        // POST /workers/heartbeat — worker heartbeat, very high limit
        new RateLimitRule
        {
            PolicyName = "worker-heartbeat",
            Limit      = 600,
            Window     = TimeSpan.FromMinutes(1),
            Paths      = ["/api/v1/workers/heartbeat"],
            Methods    = ["POST"]
        },
        new RateLimitRule
        {
